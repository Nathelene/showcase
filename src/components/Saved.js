...
export default function Saved({savedFacts,deleteSaved,error}) {
            ...
            const savedArray = savedFacts.map((fact, index) => {
                ...
                <div className="save-card-container">
                  ...
                  <button className="delete-button" onClick={() => deleteSaved(index)}><span class="material-symbols-outlined">delete</span></button>
                  </div>
                );
              });
        ...
        <div>
          {error? <h1 className="error-message">{error}</h1> :
           <div className="saved-facts-container">
            ...
            <NavLink to='/'>
            <button className="back-button"><span class="material-symbols-outlined">arrow_back</span></button>
            </NavLink>
            {savedArray}
            </div>
          }
